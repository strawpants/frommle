...
namespace frommle {
    namespace core {
        ...
        ///@brief Base class for guided Arrays. This  base class handles the guides and provides a set of virtual functions which can be overloaded in specialized classes (e.g. sparse, dense, diagonal, triangular matrices"
        ...
        public:
            ...
            GArrayBase(const guides::GuidePackPtr &guidepack) : Frommle("GArray"),
                                                               gp_(std::dynamic_pointer_cast<guides::GuidePackDyn<n>>(
                                                                       guidepack)){}
            ...
            ///@brief return internal guidepack by reference
            const gp_t &gp() const { return *gp_; }

            gp_t &gp() { return *gp_; }

            ///@brief return internal guidepack as a shared_ptr
            const gp_ptr_t &gpp() const { return gp_; }

            gp_ptr_t &gpp() { return gp_; }

            ///@brief assign array a scalar (needs to be implemented in derived classes)
            virtual GArrayBase &operator=(const T scalar)=0;
            template<class GA>
            GA as(){
                auto self=this;
                auto gaptr=dynamic_cast<GA>(self);
                if(gaptr){
                    return gaptr;
                }
                THROWINPUTEXCEPTION("cannot downcast GArray tot he specified type");
        ...
        protected:
            //note: we use a shared_ptr to store the guidepack so we can (1) store derived (speciialized) guidepacks
            // and (2) allow guidepacks to be shared(co-owned) with other objects
            gp_ptr_t gp_{};
...
//            GArrayDyn(const guides::GuidePackPtr &guidepack) : Frommle("GArray"),
//                                                               gp_(std::dynamic_pointer_cast<guides::GuidePackDyn<n>>(
//                                                                       guidepack)),
//                                                               data_(std::shared_ptr<tvec>(new T[gp_->num_elements()])),
//                                                               ar_(data_.get(), gp_->extent()) {}
