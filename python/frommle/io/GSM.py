...
class SHGSMv6Archive(SHArchive):
    ...
    def fload_impl(self):
        ...
        with self.fid() as fid:
            ...
            self.attr["nmaxfile"]=hdr["dimensions"]["degree"]
            if not "nmax" in self.attr:
                self.attr["nmax"]=self.attr["nmaxfile"]

            self.attr["tstart"]=hdr["global_attributes"]["time_coverage_start"]
            self.attr["tend"]=hdr["global_attributes"]["time_coverage_end"]

            nonstand=hdr["non-standard_attributes"]

            self.attr["gm"]=nonstand["earth_gravity_param"]["value"]
            self.attr["re"]=nonstand["mean_equator_radius"]["value"]
            ...
            for ln in fid:
                if dataregex.match(ln):
                    ...
                    # import pdb;pdb.set_trace()
                    cnm[idxc]=float(lnspl[3])
                    if m!=0:
                        cnm[idxs]=float(lnspl[4])

                    sigcnm[idxc]=float(lnspl[5])
                    if m!=0:
                        sigcnm[idxs]=float(lnspl[6])
